...
export const getMoviesByCountry = async (iso) => {
  const res = await fetch(
    `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_origin_country=${iso}&language=en-US&sort_by=popularity.desc`
  );
  const data = await res.json();
  return data.results || [];
};

// Fungsi sederhana untuk mengubah ISO menjadi Nama Negara yang rapi
