...
export async function getPersonDetails(id) {
  ...
  // Menambahkan movie_credits di sini
  const url = `https://api.themoviedb.org/3/person/${id}?api_key=${API_KEY}&language=en-US&append_to_response=movie_credits,images`;
...
export async function getArtistResults(query) {
  const url = `${BASE_URL}/search/person?api_key=${API_KEY}&query=${encodeURIComponent(query)}&language=en-US`;
  ...
  try {
    ...
    // Kita ambil 'results' karena ini adalah Array daftar orangnya
    return data.results || []; 
  } catch (error) {
    console.error(error);
    return [];
